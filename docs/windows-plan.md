...
> Superseded in part: `sasonica.mjs` is now the runner on every platform, and
> the bash runner, `lib/platform.sh` and `lib/macos-job.py` are gone. What follows
...
- `sasonica.mjs` is the runner, shared with Linux and macOS; only the process
  handling (taskkill, PowerShell, %LOCALAPPDATA%) differs on Windows.
- `install.ps1` is the installer: it provisions Cloudflare, writes
  `%APPDATA%\sasonica\{env,relay.key}` and registers a Scheduled Task. The WSL2
...
## Still open
- ~~No `--help` on Windows~~ — `sasonica --help` is in the runner now.
- No log rotation for `%APPDATA%\sasonica\runner.log`. The journal and launchd
