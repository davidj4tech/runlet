...
// The unit's file name, in the repo and in ~/.config/systemd/user alike.
export const UNIT = 'sasonica-shell.service';
...
export function installUnit({ node, runner, here, user }) {
  ...
  writeFileSync(path.join(unitDir, UNIT),
    readFileSync(path.join(here, UNIT), 'utf8')
  ...
  out.push(`${UNIT}: ${systemctl('is-active', UNIT).stdout.trim()}`);
  out.push('Runner log: journalctl --user -u sasonica-shell -f');
  return out;
}
