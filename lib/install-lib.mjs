...
export function urlSecret(words, wordsFile, note = () => {}) {
  if (!(words >= 4)) { note(`RUNLET_SECRET_WORDS=${words} is too few; using 5`); words = 5; }
...
export function renderEnv(v, today = new Date().toISOString().slice(0, 10)) {
  return `# runlet — written by install.mjs ${today}.
# The runner token here is this machine's credential for its own Worker.
CLOUDFLARE_ACCOUNT_ID=${v.accountId}
RUNLET_SITE=${v.site}
RUNLET_WORKER_NAME=${v.workerName}
RUNLET_DB_NAME=${v.dbName}
RUNLET_DB_ID=${v.dbId}
RUNLET_URL_SECRET=${v.secret}
RUNLET_WORKER_URL=${v.workerUrl}
RUNLET_RUNNER_TOKEN=${v.runnerToken}
RUNLET_POLL=5
RUNLET_CMD_TIMEOUT=600
RUNLET_KEY_FILE=${v.keyFile}
`;
}
